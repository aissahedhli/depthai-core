...
class XLinkInHost : public NodeCRTP<ThreadedHostNode, XLinkInHost> {
   private:
    ...
    std::shared_ptr<ADatatype> readData(const XLinkStream& stream);
    std::shared_ptr<ADatatype> parsePacketizedData(const std::shared_ptr<PacketizedData>& packetizedData, const XLinkStream& stream);
    void parseMessageGroup(const std::shared_ptr<MessageGroup>& messageGroup, const XLinkStream& stream);
